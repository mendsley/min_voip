...
struct Beamforming {
  ...
  Beamforming(bool enabled, const std::vector<Point>& array_geometry)
	  ...
	  , target_direction(SphericalPointf(static_cast<float>(M_PI) / 2.f, 0.f, 1.f))
      //: Beamforming(enabled,
      //              array_geometry,
      //              SphericalPointf(static_cast<float>(M_PI) / 2.f, 0.f, 1.f)) {
  {
...
private:
	RTC_DISALLOW_ASSIGN(Beamforming);
};
...
class ProcessingConfig {
 public:
  ...
  static ProcessingConfig HACK_from_stream_values(int sample_rate, int num_channels, bool has_keyboard)
  {
	  ProcessingConfig p;
	  for (int ii = 0; ii < StreamName::kNumStreamNames; ++ii)
	  {
		  p.streams[ii] = StreamConfig(sample_rate, num_channels, has_keyboard);
	  }
	  return p;
  }
};
