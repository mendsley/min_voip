...
#endif
      api_format_(ProcessingConfig::HACK_from_stream_values(kSampleRate16kHz, 1, false))/* {{{kSampleRate16kHz, 1, false},
...
int AudioProcessingImpl::Initialize(int input_sample_rate_hz,
  ...
  processing_config.streams[0] = StreamConfig(input_sample_rate_hz, ChannelsFromLayout(input_layout), LayoutHasKeyboard(input_layout));
  processing_config.streams[1] = StreamConfig(output_sample_rate_hz, ChannelsFromLayout(output_layout), LayoutHasKeyboard(output_layout));
  processing_config.streams[2] = StreamConfig(reverse_sample_rate_hz, ChannelsFromLayout(reverse_layout), LayoutHasKeyboard(reverse_layout));
  processing_config.streams[3] = processing_config.streams[2];
